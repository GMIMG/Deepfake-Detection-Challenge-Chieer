...
def predict():
    ...
    def crop_img(face_coord):
        ...
        # 이미지 전처리 및 예측
        img_tensor = (crop_img.flatten() / 255.0).reshape(-1, input_shape[0], input_shape[1], 3)
        return img_tensor
    ...
    while(cap.isOpened()):
        ...
        img_tensor = crop_img(face_coord)
        ...
        heatmap2 = cv2.resize(heatmap, (img_tensor.shape[1], img_tensor.shape[0]))
